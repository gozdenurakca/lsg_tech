  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        <Card title="Toplam Kullanıcı" value={totalUsers} desc="Sistemde kayıtlı kullanıcı" color="blue" />
        <Card title="Partner Başvuruları" value={totalApplications} desc={`${newApplications} yeni başvuru`} color="purple" />
        <Card title="Onaylı Partner" value={approvedApplications} desc={`${rejectedApplications} reddedildi`} color="green" />
        <Card title="Toplam Ürün" value={totalProducts} desc="Sistemde tanımlı ürün" color="orange" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <h2 className="font-semibold text-slate-800 mb-4">Son Kayıt Olan Kullanıcılar</h2>
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <h2 className="font-semibold text-slate-800 mb-4">Başvuru Dağılımı</h2>
...
function Card({ title, value, desc, color }: any) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border p-6 relative overflow-hidden">
      <div className={`absolute top-0 right-0 w-20 h-20 bg-gradient-to-br ${colors[color]} opacity-10 rounded-bl-3xl`} />
